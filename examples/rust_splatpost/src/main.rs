...
{
    let decoder = png::Decoder::new(File::open("splatpost.png").unwrap());
    let mut reader = decoder.read_info().unwrap();
    let mut buf = vec![0; reader.output_buffer_size()];
    let info = reader.next_frame(&mut buf).unwrap();
    let image_data = &buf[..info.buffer_size()];

    //println!("{:?} {:?}", image_data[0], image_data[3]);
    ...
    // 0xB SNES controller
    // 0xC n64
    // 0xD generic usb no sticks
    // 0xE generic usb no sticks
    // 0xF rejected?
    // 0x10-13 rejected?
    let device_type: u8 = 0x6;
    let device_subtype: u8 = 0x0; // For SNES: 00 US SNES (purple+logo), 01 JP SFC (colors+logo), 02 EU SNES (colors+logo), 03 generic round controller
    ...
    let mut printhead_x: i32 = 0;
    let mut printhead_y: i32 = 0; // 114
    let mut print_direction: i32 = 1;
    let mut print_direction_y: i32 = 0;
    let mut centered_frames: u32 = 0; // 501
    let mut frame_counter: u32 = 4;
    let mut print_buttons: u32 = 0;
...
*/
        ...
        */
        ...
        if read_bytes == 0 {
            ...
            if input_elapsed >= 16 && should_stream_input {
                input_accepted = true;
